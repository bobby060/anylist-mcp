...
// Define AnyListMCPServer

export class AnylistMCPServer extends McpServer {
  constructor(options = {}) {
    super( { name: "anylist-mcp-server",
  version: "1.0.0", ...options });
    this.anylistClient = new AnyListClient();
    ...
    this.registerTool("health_check", {
      ...
      try {
        await this.anylistClient.connect(list_name);
        return {
          content: [
            {
              type: "text",
              text: `Successfully connected to AnyList and found list: "${this.anylistClient.targetList.name}"`,
            },
          ],
        };
      } catch (error) {
    ...
    this.registerTool("add_item", {
      ...
      try {
        await this.anylistClient.connect(list_name);
        await this.anylistClient.addItem(name, quantity || 1, notes || null);
        return {
          content: [
            {
              type: "text",
    ...
    this.registerTool("check_item", {
      ...
      try {
        await this.anylistClient.connect(list_name);
        await this.anylistClient.removeItem(name);
        return {
          content: [
            {
              type: "text",
              text: `Successfully checked off "${name}" from list "${this.anylistClient.targetList.name}"`,
            },
          ],
        };
      } catch (error) {
    ...
    this.registerTool("list_items", {
      ...
      try {
        await this.anylistClient.connect(list_name);
        const items = await this.anylistClient.getItems(include_checked || false, include_notes || false);
            ...
            content: [
              {
                ...
                text: include_checked
                  ? `List "${this.anylistClient.targetList.name}" is empty.`
                  : `No unchecked items on list "${this.anylistClient.targetList.name}".`,
              },
            ],
          };
        }
        ...
        return {
          content: [
            {
              type: "text",
              text: `Shopping list "${this.anylistClient.targetList.name}" (${items.length} items):\n${itemList}`,
            },
          ],
        };
      } catch (error) {
    ...
    this.registerTool("list_lists", {
      ...
      try {
        ...
        await this.anylistClient.connect(process.env.ANYLIST_LIST_NAME || null);
        const lists = this.anylistClient.getLists();
...
export default AnylistMCPServer;


// This lets Claude desktop and code call js file directly for stdioserver transport
